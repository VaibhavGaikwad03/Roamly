...
// Roamly landing page — a full-screen, scroll-driven intro for signed-out
// visitors. Primary CTAs clear the #landing hash to drop into the auth-gated
// app (sign in / sign up); secondary CTAs scroll within the page.
export default function GlobeScrollDemo() {
  // The app locks document scrolling (body { overflow: hidden }) for its fixed
  // map shell. This page scrolls the window, so flag <body> while it's mounted
  // to lift that lock (see `body.landing-active` in index.css).
  useEffect(() => {
    document.body.classList.add('landing-active');
    return () => document.body.classList.remove('landing-active');
  }, []);

  // Hand off to the auth screen (Root shows <Auth /> on the #signin hash).
  const enterApp = () => {
    window.location.hash = 'signin';
  };

  const roamlySections = [
    {
      id: "hero",
      badge: "Welcome to Roamly",
      title: "Roamly",
      subtitle: "Every place, one map",
      description: "Track the places you've loved and the ones you're longing to see. Drop a pin, mark it visited or want-to-visit, and watch your travels come together on one beautiful, interactive map.",
      align: "left" as const,
      actions: [
        { label: "Start mapping", variant: "primary" as const, onClick: enterApp },
        { label: "See how it works", variant: "secondary" as const, onClick: () => window.scrollTo({ top: window.innerHeight, behavior: 'smooth' }) },
      ]
    },
    {
      id: "map",
      badge: "Your map",
      title: "Pinned across the globe",
      description: "From the café down the street to the trailhead across the world, every place you add lands on a live map. Pan, zoom, and see your whole story of somewhere-been and somewhere-next at a glance.",
    ...
    },
    {
      ...
      badge: "Organize",
      title: "Been there,",
      subtitle: "Going there",
      description: "Roamly keeps the two halves of travel apart so they stay clear: the places you've already explored, and the wishlist pulling you forward. Search, filter by category, and find any spot in seconds.",
      align: "left" as const,
      features: [
        { title: "Visited & want-to-visit", description: "One tap to mark a place explored or add it to your wishlist" },
        { title: "Search & filter", description: "Find any pin fast — by name, or filtered across trek and travel categories" },
        { title: "Interactive map", description: "Every place plotted and clickable, with details a tap away" }
      ]
    },
    {
      id: "plan",
      badge: "Plan ahead",
      title: "Where to next?",
      subtitle: "Let Roamly help",
      description: "Get AI-powered recommendations and trip ideas built around the places you already love. Turn a scattered wishlist into a real plan — then go add the next pin to your map.",
      align: "center" as const,
      actions: [
        { label: "Create your map", variant: "primary" as const, onClick: enterApp },
        { label: "Back to top", variant: "secondary" as const, onClick: () => window.scrollTo({ top: 0, behavior: 'smooth' }) }
      ]
    }
  ];
